...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err)){
        ...
        $sql = "SELECT idUser, pseudo, password, idEtat, idRole FROM users WHERE pseudo = ?";
        ...
        if($stmt = mysqli_prepare($db, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    mysqli_stmt_bind_result($stmt, $id, $username, $hashed_password, $etat, $role);
                    if(mysqli_stmt_fetch($stmt)){
                        //Vérifier si le compte n'est pas banni ou en cours de suppression
                        if($etat!=1){
                            $login_err = "Vous n'êtes pas autorisé à vous connecter à votre espace utilisateur.";
...
function getIp(){
    ...
    //}else if(!empty($_SERVER['REMOTE_ADDR'])){
        ...
        $ip1 = "127.0.0.1"; //adresse IP du serveur
    }
    return $ip1;
}
$ip=getIp();

//$ip=$_SERVER['HTTP_X_FORWARDED_FOR']; ==> à activer sur un serveur web
$stmt = $db->prepare("INSERT INTO connexions(dateCo,adresseIP,idUser) VALUES(NOW(), ?, ?)");
